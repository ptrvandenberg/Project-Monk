...
#    x_xr = LpVariable.dicts("roster_%s_%s", (members, days), 0, 1, LpBinary)
    ...
    for m in members.index:
#        model += lpSum([x[m][d][s] for d in days.index s in shifts.index if s.ix['value'] <> "XP" and s.ix['value'] <> "XP"]) == settings.ix['nbr_roster_weeks','value'] * 5 * members['fte']
        model += lpSum([x[m][d][s] for d in days.index s in shifts.index] == settings.ix['nbr_roster_weeks','value'] * 5 * members['fte']
