...
def solve(dat):
                ...
                for d in range(1,7+1):
                    if isnull(predetermined.ix[m].ix[w,d-1]):
                        if not isnull(longshifts.ix[m].ix[ls,d-1]):
                            predetermined['d'+str(d)].loc[m,w] = longshifts.ix[m].ix[ls,d-1]
    ...
    crew_pm_bin1 = LpVariable.dicts("crew_pm_bin1_d%s", days, 0, 1, LpBinary)
    crew_pm_bin2 = LpVariable.dicts("crew_pm_bin2_d%s", days, 0, 1, LpBinary)
    crew_pm_bin3 = LpVariable.dicts("crew_pm_bin3_d%s", days, 0, 1, LpBinary)
...
#                roster.ix[m,'week'] = 1 + int(int(d)/7)
                roster.ix[m].ix[unit].ix[period].ix[1 + int(int(d)/7),'carryin_rest'] = carryover.ix[m,'r0_co_rests']
